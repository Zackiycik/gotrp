...
module.exports = {
    data: new Discord.SlashCommandBuilder()
        .setName("askersil")
        .setDescription("Kullanıcıya asker silin!")
        .setDefaultMemberPermissions(Discord.PermissionFlagsBits.Administrator)
        .addUserOption(option =>
            option.setName('user')
                .setDescription('Asker silmek istenen kullanıcıyı seçin.')
                .setRequired(true))
        .addStringOption(option =>
            option.setName('tur')
            .setDescription('Silmek istediğin askeri seç!')
            .setRequired(true)
            .addChoices(
        ...
        .addIntegerOption(z => z.setName("miktar").setDescription("Sileceğin asker miktarını girmelisin.").setRequired(true))
    ,
    run: async (client, interaction) => {
        ...
        db.push(`${haneId}`,{...veri, adet:parseInt(asker.adet)-parseInt(miktar)});
        interaction.reply({content: `**${veri.isim}** adındaki askerlerden ${miktar} adet ${user} adlı kullanıcıdan çıkarıldı.`})
    }
};