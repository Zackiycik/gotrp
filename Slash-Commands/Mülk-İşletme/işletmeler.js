...
module.exports = {
    data: new Discord.SlashCommandBuilder()
        .setName("işletmelerim")
        .setDescription("Kullanıcının işletmelerini gösterir.")
        .setDMPermission(false)
        .addUserOption(z => z.setName("etiket").setDescription("İşletmelerine bakmak istediğin kişiyi etiketle."))
        , 
    run: async (client, interaction) => {
        let user = interaction.options.getUser("etiket") || interaction.user;
        let inv = db.get(`profil.${user.id}.isletmeler`) || [];
        if(inv.length == 0) return interaction.reply({content: "İşletmen yok gibi duruyor."})
        ...
        .setColor("000000")
        .setDescription(`**${user} Adlı Kullanıcının İşletmeler:**\n\n${liste}`)
